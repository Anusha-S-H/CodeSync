...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const { setUsers, drawingData, users } = useAppContext()

    // Load file structure from localStorage on mount
    const loadFileStructureFromStorage = (): FileSystemItem => {
        try {
            const stored = localStorage.getItem('code_sync_fileStructure')
            if (stored) {
    ...
    const loadOpenFilesFromStorage = (): FileSystemItem[] => {
        try {
            const stored = localStorage.getItem('code_sync_openFiles')
            if (stored) {
    ...
    const loadActiveFileFromStorage = (): FileSystemItem | null => {
        try {
            const stored = localStorage.getItem('code_sync_activeFile')
            if (stored) {
                return JSON.parse(stored)
            }
        } catch (error) {
    ...
    const [fileStructure, setFileStructure] =
        useState<FileSystemItem>(loadFileStructureFromStorage)
    
    const storedOpenFiles = loadOpenFilesFromStorage()
    const initialOpenFiles = storedOpenFiles.length > 0 
        ? storedOpenFiles 
        : (fileStructure.children ? fileStructure.children : [])
    
    const [openFiles, setOpenFiles] =
        useState<FileSystemItem[]>(initialOpenFiles)
    
    const storedActiveFile = loadActiveFileFromStorage()
    const [activeFile, setActiveFile] = useState<FileSystemItem | null>(
        storedActiveFile || openFiles[0] || null,
    )
    ...
    const handleFileStructureSync = useCallback(
        ({
            ...
            // Only sync if we receive valid data from another user
            // This prevents overwriting localStorage with empty data
            if (fileStructure && fileStructure.id) {
                // Check if the incoming structure has actual content (children/files)
                const hasContent = fileStructure.children && fileStructure.children.length > 0
                const currentHasContent = fileStructure.children && fileStructure.children.length > 0
                
                // Only overwrite if incoming data has content or we have no content currently
    ...
    useEffect(() => {
        try {
            ...
            if (fileStructure && fileStructure.id) {
                localStorage.setItem('code_sync_fileStructure', JSON.stringify(fileStructure))
            }
        } catch (error) {
            console.error('Failed to save file structure to localStorage:', error)
        }
    }, [fileStructure])
    ...
    useEffect(() => {
        try {
            localStorage.setItem('code_sync_openFiles', JSON.stringify(openFiles))
        } catch (error) {
            console.error('Failed to save open files to localStorage:', error)
        }
    }, [openFiles])
    ...
    useEffect(() => {
        try {
            if (activeFile) {
                localStorage.setItem('code_sync_activeFile', JSON.stringify(activeFile))
                // Also update the content in fileStructure to ensure it's saved
                if (activeFile.content !== undefined) {
                    const updateContent = (directory: FileSystemItem): FileSystemItem => {
                        if (directory.type === "file" && directory.id === activeFile.id) {
                            return { ...directory, content: activeFile.content }
                        } else if (directory.children) {
                            return {
                                ...directory,
                                children: directory.children.map(updateContent),
                            }
                        }
                        return directory
                    }
                ...
                localStorage.removeItem('code_sync_activeFile')
            }
        } catch (error) {
